...
var ttsModels = []ModelOption{
	...
	{ID: "qwen", Name: "Qwen3-TTS", Seconds: 200.696, Speedup: 1, LoadSeconds: 9.244, WarmupSeconds: 17.571},
}
...
func normalizeModelIDs(tts, translation *string) error {
	...
	if *translation == "" {
		*translation = defaultTranslationModel
	}
	if _, ok := findModel(ttsModels, *tts); !ok {
		return fmt.Errorf("unknown TTS model: %s", *tts)
	}
	if _, ok := findModel(translationModels, *translation); !ok {
		return fmt.Errorf("unknown translation model: %s", *translation)
	}
	return nil
}
